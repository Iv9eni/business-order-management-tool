<!--
	Author: Ivgeni Darinski
	Student NO.: 250920885
	Date Created: 2018-11-07
	File: index.php
-->

<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8">
		<title>EBAY Admin Tools Index</title>
		<link rel="stylesheet" type="text/css" href="styling/defaultstyle.css" />
	</head>
	<body>
    <!-- EBAY logo -->
    <img id="logo" src="images\logo.png">

		<!-- Allows navigation through website -->
		<div id="navigationBar">
			<ul>
				<li><a class="active" href="index.php">Home</a></li>
				<li><a href="ViewCustomerPurchases.php">View Customer Purchases</a></li>
				<li><a href="Products.php">Products</a></li>
				<li><a href="CreateBuyOrder.php">Create Buy Order</a></li>
				<li><a href="AddDeleteCustomers.php">Add/Delete Customers</a></li>
				<li><a href="UpdatePhoneNumber.php">Update Phone</a></li>
				<li><a href="OrderSummary.php">Order Summary</a></li>
			</ul>
		</div>

		<br><br>

		<!-- Info to the TA's -->
		<p> "View Customer Purchases" page contains Question 1) results </p><br>
		<p> "Products" page contains the results of Question 2), 7), 8)</p><br>
		<p> "Create Buy Order" page contains the results of Question 3)</p><br>
		<p> "Add/Delete Customers" page contains the results of Question 4) and 6) </p><br>
		<p> "Update Phone" page contains the results of Question 5) </p><br>
		<p> "Order Summary" page contains the results of Question 9) </p><br>

	</body>
</html>
